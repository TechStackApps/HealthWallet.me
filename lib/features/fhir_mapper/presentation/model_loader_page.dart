...
class _ModelLoaderPageState extends State<ModelLoaderPage>
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<FhirMapperBloc, FhirMapperState>(
      ...
      listener: (context, state) {
        if (state.status == FhirMapperStatus.modelLoaded) {
          context.router.replace(const TextInputRoute());
        }
      },
      child: Scaffold(
        appBar: AppBar(
  ...
  Widget _buildView(FhirMapperState state) {
    if (state.status == FhirMapperStatus.modelLoading &&
      ...
      children: [
        ...
        if (state.status != FhirMapperStatus.modelLoading) ...[
          ...
          SizedBox(
            ...
            child: TextButton(
              onPressed: () {},
              child: const Text("Scan Without AI Instead"),
            ),
          ),
        ] else
