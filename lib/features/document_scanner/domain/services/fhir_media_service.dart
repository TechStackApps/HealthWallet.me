...
import 'package:health_wallet/features/document_scanner/domain/models/fhir_media.dart';
...
class FhirMediaService {
  ...
  Future<List<FhirMedia>> getFhirMediaForEncounter(String encounterId) async {
    ...
    return allMedia
        .where((media) => media.encounter?.reference == 'Encounter/$encounterId')
        .toList();
  }
  ...
  Future<void> deleteFhirMedia(String mediaId) async {
    try {
      final directory = await getApplicationDocumentsDirectory();
      final filePath = path.join(directory.path, _fhirMediaDirectory, 'media_$mediaId.json');
      final file = File(filePath);
